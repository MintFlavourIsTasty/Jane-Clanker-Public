...
class ButtonRoleActionButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        botUser = getattr(interaction.client, "user", None)
        botMember = interaction.guild.me
        if botMember is None and botUser is not None:
            botMember = interaction.guild.get_member(int(getattr(botUser, "id", 0) or 0))
        if (
            role is None
            or botMember is None
            or role.managed
            or role.is_default()
            or role.position >= botMember.top_role.position
